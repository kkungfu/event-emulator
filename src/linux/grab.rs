...
use crate::emulator::{Button, Event, EventType, GrabError, Key, KeyboardState};
...
macro_rules! convert_keys {
    ($($ev_key:ident, $emulator_key:ident),*) => {
        //TODO: make const when rust lang issue #49146 is fixed
        #[allow(unreachable_patterns)]
        fn evdev_key_to_emulator_key(key: &EV_KEY) -> Option<Key> {
            match key {
                $(
                    EV_KEY::$ev_key => Some(Key::$emulator_key),
                )*
                _ => None,
            }
        }
        ...
        // //TODO: make const when rust lang issue #49146 is fixed
        // fn emulator_key_to_evdev_key(key: &Key) -> Option<EV_KEY> {
        //     match key {
        //         $(
        //             Key::$emulator_key => Some(EV_KEY::$ev_key),
        //         )*
        //         _ => None
        //     }
        // }
...
macro_rules! convert_buttons {
    ($($ev_key:ident, $emulator_key:ident),*) => {
        //TODO: make const when rust lang issue #49146 is fixed
        fn evdev_key_to_emulator_button(key: &EV_KEY) -> Option<Button> {
            match key {
                $(
                    EV_KEY::$ev_key => Some(Button::$emulator_key),
                )*
                _ => None,
            }
        }
        ...
        // //TODO: make const when rust lang issue #49146 is fixed
        // fn emulator_button_to_evdev_key(event: &Button) -> Option<EV_KEY> {
        //     match event {
        //         $(
        //             Button::$emulator_key => Some(EV_KEY::$ev_key),
        //         )*
        //         _ => None
        //     }
        // }
...
fn evdev_event_to_emulator_event(
    event: &InputEvent,
    x: &mut f64,
    y: &mut f64,
    w: f64,
    ...
    match &event.event_code {
        ...
        EventCode::EV_REL(mouse) => match mouse {
            ...
            // Other EV_REL events cannot be represented by emulator
            _ => None,
        },
        // Other event_codes cannot be represented by emulator,
        // and some never will e.g. EV_SYN
        _ => None,
    }
}

// fn emulator_event_to_evdev_event(event: &EventType, time: &TimeVal) -> Option<InputEvent> {
//     match event {
//         EventType::KeyPress(key) => {
//             let key = emulator_key_to_evdev_key(&key)?;
...
//             let button = emulator_button_to_evdev_key(&button)?;
    ...
    T: Fn(Event) -> Option<Event> + 'static,
{
    ...
    filter_map_events(|event| {
        let event_type = match evdev_event_to_emulator_event(&event, &mut x, &mut y, w, h) {
            Some(emulator_event) => emulator_event,
            // If we can't convert event, simulate it
            None => return (Some(event), GrabStatus::Continue),
        };
        let name = kb.add(&event_type);
        let emulator_event = Event {
            time: SystemTime::now(),
            name,
            event_type,
        };
        if callback(emulator_event).is_some() {
